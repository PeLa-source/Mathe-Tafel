...
const state={
  ...
  games:{bingoSettings:{range:20,type:'mixed'},riddleSettings:{range:20,type:'mixed',showLine:false},middleSettings:{range:20},matchSettings:{representations:['symbol','word','twenty','ttg']},discoverSettings:{range:20,op:'addition',pattern:'oneUp'}},
...
function topbar(){
  ...
  const configurable=['workspace','twentyTool','lineTool','decompose','placeMenu','ttg','house','material','placevalue','connect','game','bingo','riddle','middle','match','discover'].includes(state.route);
...
const reprNames={symbol:'Zahlsymbol',word:'Zahlwort',material:'Eisstiele',dienes:'Dienes',twenty:'Zwanzigerfeld',language:'Zehner und Einer',short:'Kurzschreibweise',table:'Stellenwerttafel',ttg:'Teil–Teil–Ganzes',numberline:'Zahlenstrahl',hundred:'Hundertertafel'};
function connectContent(id){
  ...
  if(id==='language')return el('div',{class:'language-display'},d?(d.tens===0?`${d.ones} Einer`:`${d.tens} Zehner und ${d.ones} Einer`):'einhundert');
...
function drawer(){
  ...
  if(r==='match'){const ids=['symbol','word','language','short','table','twenty','dienes','ttg'];const chosen=state.games.matchSettings.representations;content.push(el('div',{class:'section'},'Darstellungen zuordnen'),el('p',{class:'drawer-note'},'Wähle genau vier Darstellungen für die vier Karten.'),...ids.map(id=>toggle(reprNames[id],chosen.includes(id),v=>{if(v&&!chosen.includes(id)){if(chosen.length>=4)chosen.shift();chosen.push(id);}if(!v&&chosen.includes(id))state.games.matchSettings.representations=chosen.filter(x=>x!==id);state.games.match=null;render();})),el('p',{class:'drawer-note'},`${state.games.matchSettings.representations.length}/4 ausgewählt`));}
...
function renderMiddle(){if(!state.games.middle)newMiddle();const g=state.games.middle;const line=clickableNumberLine(g.a,g.b,v=>{g.selected=v;g.checked=false;render();},g.selected,g.reveal?g.m:null);line.classList.add('unscaled');const why=g.reveal?el('div',{class:'middle-why'},el('span',{},`${g.a} → ${g.m}: ${g.m-g.a} Schritte`),el('strong',{},'gleich weit'),el('span',{},`${g.m} → ${g.b}: ${g.b-g.m} Schritte`)):null;return stage('Finde die Mitte',el('div',{class:'routine middle-routine'},el('div',{class:'prompt'},'Welche Zahl liegt genau in der Mitte?'),line,g.checked?feedbackBox(g):null,why),[btn('PRÜFEN',()=>{if(g.selected!=null){g.checked=true;render();}},'action soft',{disabled:g.selected==null}),btn('LÖSUNG ZEIGEN',()=>{g.reveal=true;render();},'action soft'),btn('NEUE AUFGABE',()=>{newMiddle();render();},'action primary')]);}
function newMatch(error=false){
 const target=randomInt(1,20),configured=state.games.matchSettings.representations||['symbol','word','twenty','ttg'];
 const ids=[...configured].slice(0,4);
 while(ids.length<4){for(const id of ['symbol','word','twenty','ttg']){if(!ids.includes(id)){ids.push(id);break;}}}
 if(error){state.games.match={target,ids,error:true,bad:randomInt(0,3),selected:[],checked:false,reveal:false};}
 else {const bad=randomInt(0,3);state.games.match={target,ids,error:false,correct:[0,1,2,3].filter(i=>i!==bad),bad,selected:[],checked:false,reveal:false};}
}
